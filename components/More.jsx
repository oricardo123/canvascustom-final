...
const More = ({currentCursor, size=1, catalogSlug}) => {
    ...
    const getMore = async () => {
        setLoading(true)
        const {productsConnection} = await fetchProductsConnection(cursor, size, catalogSlug)
    ...
    return (
        ...
        {products.map((product) => (
            <div key={product.cursor} className="prose bg-white  max-w-4xl mb-4 p-4 rounded-md">
